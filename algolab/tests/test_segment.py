...
class SegmentTest(unittest2.TestCase):
    ...
    def test_already_segmented(self):
        self.assertEqual(list(S(self.col0).segments)[0], range(0, len(npoints[2])))

    def test_already_segmented2(self):
        create_rg(npoints[3], self.col1)
        self.assertEqual(list(S(self.col1).segments)[0], [4, 2, 5])

    def test_already_segmented3(self):
        create_rg(npoints[4], self.col1)
        self.assertEqual(list(S(self.col1).segments)[0], [6, 2])
    ...
    def test_switch_segment2(self):
        ...
        self.assertItemsEqual(list(S(self.col0).segments),
                [[0, 1, 2], [2, 3], [2, 4], [2, 5],
                    [2, 6], [2, 8, 7], [2, 9, 10, 11]])
    ...
    def test_switch_segment3(self):
        ...
        self.assertEqual(len(list(S(self.col0).segments)), 12)
